...
def pandas_revenue_by_region(sales: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    revenue = sales.assign(revenue=sales["quantity"] * sales["unit_price"])
    return revenue.groupby("region", as_index=False)["revenue"].sum()
...
def pandas_region_share(sales: pd.DataFrame) -> pd.DataFrame:
    ...
    grand_total = pd.DataFrame({"total": [revenue_by_region["revenue"].sum()]})
    result = revenue_by_region.merge(grand_total, how="cross")
